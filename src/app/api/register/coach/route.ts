...
async function storeFile(file: File) {
  ...
  return { fileName: file.name, mimeType: file.type, size: file.size, data: bytes };
}
...
export async function POST(req: Request) {
  try {
    ...
    if (age < 5 || age > 75) {
      return NextResponse.json({ error: "DOB is out of valid coach range" }, { status: 400 });
    }
    ...
    const coach = await Coach.create({
      ...parsed,
      personalDetails: { ...parsed.personalDetails, age, ageGroup: getAgeGroup(age) },
      documents: {
    ...
    return NextResponse.json({ id: coach._id.toString(), message: "Registration submitted successfully" });
  } catch (error) {
    console.error("POST /api/register/coach failed:", error);
