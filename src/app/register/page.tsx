...
export default function RegistrationPage() {
  ...
  const sendOtp = async () => {
    ...
    try {
      const res = await fetch("/api/otp/send", {
        ...
        body: JSON.stringify({ email: form.personalDetails.email }),
      });
  ...
  const verifyOtp = async () => {
    ...
    try {
      const res = await fetch("/api/otp/verify", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: form.personalDetails.email, otp }),
      });
      const payload = await res.json();
  ...
  const handleFile = async (key: "photoId" | "dobProof" | "medicalCertificate", e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!["image/png", "image/jpeg", "image/jpg"].includes(file.type)) {
        throw new Error("Only JPG/PNG images or PDF files are allowed");
      }
  ...
  const submit = async (e: FormEvent) => {
    ...
    if (!files.photoId || !files.dobProof || !files.medicalCertificate) {
      setError("All mandatory documents are required");
      return;
    }
    ...
    <main className="min-h-screen bg-[linear-gradient(130deg,_#ecfdf5_0%,_#f8fafc_35%,_#ffffff_100%)] px-4 py-8 md:px-6">
      <div className="mx-auto max-w-5xl">
        <div className="mb-6 flex items-center justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest text-emerald-700">Athlete Intake</p>
            <h1 className="text-3xl font-black text-slate-900 md:text-4xl">Registration Form</h1>
          </div>
        ...
        <div className="mb-4 grid grid-cols-2 gap-2 rounded-2xl border border-emerald-200 bg-white p-2 md:grid-cols-6">
          {steps.map((s, i) => (
            <div key={s} className={`rounded-xl px-2 py-2 text-center text-xs font-semibold ${i === step ? "bg-emerald-700 text-white" : "text-slate-500"}`}>
              {i + 1}. {s}
            </div>
          ))}
        </div>
        ...
        <form onSubmit={submit} className="space-y-4 rounded-3xl border border-slate-200 bg-white p-5 shadow-[0_30px_80px_-40px_rgba(15,23,42,0.45)] md:p-8">
          ...
          {step === 5 && (
            <div className="grid gap-3">
              <label className="text-sm font-semibold">Photo ID (JPG/PNG {"<="}1MB, PDF {"<="}2MB)<input className={`${fieldStyle} mt-1`} type="file" accept="image/png,image/jpeg,application/pdf" onChange={(e) => handleFile("photoId", e)} required /></label>
              <label className="text-sm font-semibold">DOB Proof (JPG/PNG {"<="}1MB, PDF {"<="}2MB)<input className={`${fieldStyle} mt-1`} type="file" accept="image/png,image/jpeg,application/pdf" onChange={(e) => handleFile("dobProof", e)} required /></label>
              <label className="text-sm font-semibold">Medical Certificate (JPG/PNG {"<="}1MB, PDF {"<="}2MB)<input className={`${fieldStyle} mt-1`} type="file" accept="image/png,image/jpeg,application/pdf" onChange={(e) => handleFile("medicalCertificate", e)} required /></label>
            </div>
          )}
